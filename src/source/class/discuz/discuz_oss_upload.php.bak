...
Class discuz_oss_upload{

	global $_G;
	require_once('../../plugin/oss4dz/lib/sdk.class.php');
	var	$obj = new ALIOSS($_G['cache']['plugin']['oss4dz']['key'],$_G['cache']['plugin']['oss4dz']['secret']);
	...
	function save_to_local($source, $target) {
		...
		$a.="\r\ntarget:".$target;
		$a.="\r\n";
		$objects=split('/data/attachment/./',$target);

		$f=fopen('D:\vhosts\dzbbs.aliyun.com\source\class\discuz\logx.txt','a');
		fwrite($f,$a);
		fwrite($f,$objects[count($objects)-1]."\r\n");
		fclose($f);
		die();
		*/
		...
		if(!discuz_upload::is_upload_file($source)) {
			...
			$response = $obj->upload_file_by_content($_G['cache']['plugin']['oss4dz']['bucket'],$object,$upload_file_options);
