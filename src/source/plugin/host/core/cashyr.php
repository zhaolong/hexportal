...
if($array[extcredits4]<$array_plan[yrprice]){
	showmessage($lang[cash_not_enough_3],'./home.php?mod=spacecp&ac=credit&op=buy');
}else{
	$c=$array[extcredits4]-$array_plan[yrprice];
	$new_expire=$array_account[expiredate]+3600*24*GetYearDay(date('Y'));
	DB::query("UPDATE host_user SET expiredate = '$new_expire' WHERE uid='$discuz_uid' AND id='".intval($_GET['hid'])."'");
	updatemembercount($discuz_uid, array('extcredits4' => (0-$array_plan[yrprice])), true, 'R12', 12, '主机续费_12个月');
	showmessage("您成功续费1年",'./plugin.php?id=host:host&page=myhost&hid='.$_GET['hid']);
}
?>